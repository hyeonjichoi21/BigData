...
wine = pd.concat([red_df, white_df]) # 위아래로 합침
...
stats.ttest_ind(red_wine_quality, white_wine_quality, equal_var = False) # 독립성 테스트?

Rformula = 'quality ~ fixed_acidity + volatile_acidity + citric_acid + residual_sugar + chlorides + free_sulfur_dioxide + total_sulfur_dioxide + density + pH + sulphates + alcohol'

regression_result = ols(Rformula, data = wine).fit() # 예측을 한다
...
sample1 = sample1[0:5][:]
sample1_predict = regression_result.predict(sample1) # 예측하기 
sample1_predict
wine[0:5]['quality']
